...
# html 문서에서 {{ python code }},{%=변수명%} 해당하는 부분은 찾아서 형태를 변환
# html문서를 불러오는 기본경로가 현재 경로에서 templates 하위 폴더 
...
def index_second():
    ...
    # requests.args 는 dict 형태의 데이터
    print(request.args)
    _text = request.args['input_text']
    _pass = request.args['input_pass']
    print(f"유저가 입력한 text 는 {_text}이고 password는 {_pass}이다.")
    # _text가 'test'이고 _pass 가 '1234'와 같다면 로그인이 성공
    # second.html을 보여준다
    if( _text == 'test') & (_pass == '1234'):
        ...
        return render_template("second.html", 
                               ...
                               y_data = y)
        # 로그인 페이지로 되돌아간다.
        # 127.0.0.1:5000/ 에 해당하는 주소로 이동한다
    else :
         return redirect('/')
...
def third():
    print(redirect)
    # post 방식으로 데이터를 보낸다면
    # request 안에 form 에 데이터가 존재
    _text = request.form['input_text']
    _pass = request.form['input_pass']
    print(f"입력한 txt는 {_text}이고 입력한 password는 {_pass}입니다.")
    return ""

    

# 웹 서버를 시작한다
# host 매개변수 -> 허용주소 목록 (기본값은 로컬 피씨만 접속, 0.0.0.0 변경 시 보든 주소에서 접속 가능)
...
# 유저가 보낸 데이터를 서버가 확인 -> 요청 메세지 안에 데이터가 존재